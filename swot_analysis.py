...
def geostrophic_velocity(ds, dx, dy, flip=False):

    da = ds.simulated_true_ssh_karin

    omega = 7.2921e-5 / u.s
    detadx, detady = np.gradient(da, dx, dy, axis=(0, 1))

    if flip == True:
        detadx, detady = -detadx, -detady

    ug = -(c.g0.si / (2 * omega * np.sin(da.latitude * np.pi / 180))) * detady
    vg = (c.g0.si / (2 * omega * np.sin(da.latitude * np.pi / 180))) * detadx
...
def rotational_parameters(ds, dx, dy):
    ...
    dudx, dudy = np.gradient(ug, dx, dy, axis=(0, 1))
    dvdx, dvdy = np.gradient(vg, dx, dy, axis=(0, 1))
...
def sf_run(
    ...
    if SFtype == "trad":
        ...
        sfs, processed_ds = adv_sf(
            ds_dict, latmin, latmax, lonmin, lonmax, numl)
